...
the most prevelant action in a video (DeepMind Kinetic 700). To accomplish our goal, the model can be extended to classify events beyond
movement patterns. The classified events can then be fed through an embedding model to allow for a greater vocabulary,
at which point, a language model can be used to generate a sentence that best encapsulates the events that occured.

\subsection{3D ResNet}
On the same dataset mentioned above, \cite{du2021revisiting} was able to use a 3D ResNet for marginally better performance.
The same classification was done with a different architecture. As a result, the process mentioned above can be, by large,
carried over.

\subsection{BERT}
VideoBERT is an extension of the BERT architecture that additionally utilizes video tokens \cite{sun2019videobert}. 
The model found great success in the completion of masked sentences given a video clip. However, similar to the 
above mentioned models, the dataset used was extremely large. However, the model was still able to perform within reason on training sets
with $5000$ video clips, making this an interest model to explore the impact of data augmentation, regularization and normalization 
on performance. Furthemore, as our task is to generate a sentence, it may be worth exploring if part of speech tags can be applied 
to prevent the need for prefilling sentences.
